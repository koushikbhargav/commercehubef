...
export default function AutoConfig() {
  ...
  const productCount = store?.stats?.products || 0;

  const steps = [
    { icon: Server, text: `Connecting to ${storeName} API...` },
    { icon: Box, text: `Scanning catalog (${productCount.toLocaleString()} items found)...` },
  ...
  useEffect(() => {
    const timer = setInterval(() => {
      setStep(s => {
        if (s >= steps.length - 1) {
          clearInterval(timer);
          setTimeout(() => router.push(`/demo/payment/${platform || 'shopify'}`), 1000);
          return s;
        }
        return s + 1;
      });
        ...
        <div className="space-y-6 text-left max-w-md mx-auto">
          {steps.map((s, i) => (
            <motion.div
              ...
              className={`flex items-center p-4 rounded-lg border transition-colors ${i === step ? 'bg-black/5 border-black/10' :
            ...
            >
              ...
              <span className={`font-medium ${i < step ? 'text-green-900' :
                  i === step ? 'text-black' :
                    'text-slate-400'
                }`}>
                {s.text}
